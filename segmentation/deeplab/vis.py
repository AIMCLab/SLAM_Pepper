...
flags.DEFINE_enum('colormap_type', 'pascal', ['pascal', 'cityscapes', 'nyuv2', 'nyuv2_8', 'pepper'],
...
# The format to save label
_LABEL_FORMAT = '%s_%06d_label'

# The format to save prediction
_PREDICTION_FORMAT = '%s_%06d_prediction'
...
def _process_batch(sess, original_images, labels, semantic_predictions, image_names,
  ...
  for i in range(num_image):
    ...
    basename = os.path.basename(image_names[i]).decode()

    # Save image.
    save_annotation.save_annotation(
        original_image, save_dir, _IMAGE_FORMAT % (basename, image_id_offset + i),
        add_colormap=False)

    # Save label.
    save_annotation.save_annotation(
        crop_label, save_dir, _LABEL_FORMAT % (basename, image_id_offset + i), add_colormap=True,
        colormap_type=FLAGS.colormap_type)

    # Save prediction.
    save_annotation.save_annotation(
        crop_semantic_prediction, save_dir,
        _PREDICTION_FORMAT % (basename, image_id_offset + i), add_colormap=True,
        colormap_type=FLAGS.colormap_type)
    ...
    if FLAGS.also_save_raw_predictions:
      if train_id_to_eval_id is not None:
        crop_semantic_prediction = _convert_train_id_to_eval_id(
            ...
            train_id_to_eval_id)
      save_annotation.save_annotation(
          crop_semantic_prediction, raw_save_dir, basename,
          add_colormap=False)
