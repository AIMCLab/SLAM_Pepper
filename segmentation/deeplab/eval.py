...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    acc = tf.metrics.accuracy(predictions, labels, weights=weights)

    update_op = tf.Print(update_op, [miou, acc[1]], 'mIoU/Pixel accuracy: ')

    tf.summary.scalar('mean_iou/eval_mean_iou' + tag_succ, miou)
    tf.summary.scalar('px_accuracy/eval_px_accuracy' + tag_succ, acc[0])
