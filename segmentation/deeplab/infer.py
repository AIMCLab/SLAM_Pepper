...
parser.add_argument('--colormap', '-c', choices=['nyuv2', 'nyuv2_8', 'pascal_voc', 'pepper'],
                    help='the type of colormap')
parser.add_argument('--resize', '-s', type=int, default=512,
...
def get_image_list(path):
    ...
    return sorted(images, key=lambda fname: fname[:-4])
...
if __name__ == '__main__':
    ...
    for i, fname in enumerate(image_list):
        ...
        ifname = os.path.join(args.input, fname)
        ...
        Image.fromarray(pred_color.astype(dtype=np.uint8)).save(ofname)
        if i == 0:
            begin = time.time()

    end = time.time()
    print('Total time: %.3f s' % (end - begin))
    print('Frame rate: %.3f FPS' % ((len(image_list) - 1)/ (end - begin)))
