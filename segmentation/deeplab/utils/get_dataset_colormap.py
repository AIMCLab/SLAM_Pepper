...
LABEL_VOC = [(20, 20, 20),  # 0=background
...
LABEL_VOC.extend([(0, 0, 0) for i in range(255)])

LABEL_NYUV2 = [
  (20 , 20 , 20 ), # Background    DARK GREY
  (0  , 128, 128), # Bed           TEAL
  (250, 50 , 50 ), # Bookshelf     DARK RED   = object
  (102, 0  , 204), # Ceiling       PURPLE
  (50 , 50 , 250), # Chair         BRIGHT BLUE
  (220, 220, 220), # Floor         LIGHT GREY
  (255, 69 , 20 ), # Furniture     ORANGE
  (255, 20 , 127), # Objects       PINK
  (50 , 50 , 150), # Picture       DARK BLUE    = object
  (222, 180, 140), # Sofa          TAN          = chair
  (50 , 250, 50 ), # Table         BRIGHT GREEN = furniture
  (255, 215, 0  ), # Tv            GOLD         = object
  (150, 150, 150), # Wall          GREY
  (0  , 255, 255), # Window        CYAN
]

LABEL_NYUV2_8= [
  (20 , 20 , 20 ), # Background    DARK GREY
  (0  , 128, 128), # Bed/Door      TEAL
  # (250, 50 , 50 ), # Bookshelf     DARK RED   = object
  (102, 0  , 204), # Ceiling       PURPLE
  (50 , 50 , 250), # Chair         BRIGHT BLUE
  (220, 220, 220), # Floor         LIGHT GREY
  (250, 139, 0), # Furniture     ORANGE
  (255, 20 , 127), # Objects       PINK
  # (50 , 50 , 150), # Picture       DARK BLUE    = object
  # (222, 180, 140), # Sofa          TAN          = chair
  # (50 , 250, 50 ), # Table         BRIGHT GREEN = furniture
...
def create_label_colormap(dataset=_PASCAL):
  ...
  if dataset == _ADE20K:
    ...
    return np.asarray(LABEL_NYUV2)
  elif dataset == _NYUV2_8:
    return np.asarray(LABEL_NYUV2_8)
  elif dataset == _PEPPER:
    return np.asarray(LABEL_PEPPER)
  else:
...
def label_to_color2(label, dataset):
  if dataset == 'pascal_voc_seg':
    label_map = LABEL_VOC
  elif dataset == 'nyuv2':
    label_map = LABEL_NYUV2
  elif dataset == 'nyuv2_8':
    label_map = LABEL_NYUV2_8
  elif dataset == 'pepper':
    label_map = LABEL_PEPPER
  else:
    raise ValueError('Unsupported dataset.')
  return  tf.cast(tf.gather(label_map, label), tf.uint8)
