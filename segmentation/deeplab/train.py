...
from deeplab.utils import train_utils, get_dataset_colormap
...
def _log_summaries(input_image, label, num_of_classes, output, ignore_label):
  ...
  logits = tf.image.resize_bilinear(output, preprocess_utils.resolve_shape(label, 4)[1:3])
  pred = tf.argmax(logits, 3)
  label = tf.squeeze(label, [3])

  weights = tf.to_float(tf.not_equal(label, ignore_label))
  label_valid = tf.where(tf.equal(label, ignore_label), tf.zeros_like(label), label)

  acc = tf.metrics.accuracy(label_valid, pred, weights=weights)
  miou, update_op = tf.metrics.mean_iou(label_valid, pred, num_of_classes, weights=weights)
  ...
  if FLAGS.save_summaries_images:
    ...
    summary_label = get_dataset_colormap.label_to_color2(label, FLAGS.dataset)
    summary_predictions = get_dataset_colormap.label_to_color2(pred, FLAGS.dataset)

    img = tf.concat(axis=2, values=[img, summary_label, summary_predictions])
